...
class RegistrationForm(FlaskForm):
    ...
    password = PasswordField('Пароль', [DataRequired(), Length(min=8, max=200)])
    confirm_password = PasswordField('Подтверждение пароля', [DataRequired(), Length(min=8, max=200), EqualTo('password')])
